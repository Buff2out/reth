...
/// Converts a Block Access List into a [`HashedPostState`] in parallel using rayon.
///
/// Each rayon thread lazily builds its own provider and processes a subset of the BAL accounts.
/// Results are collected into per-thread partial states and merged at the end.
pub(crate) fn par_bal_to_hashed_post_state<F>(
    bal: &BlockAccessList,
    provider_factory: &F,
) -> Result<HashedPostState, ProviderError>
where
    F: Sync,
    for<'a> &'a F: AccountReaderFactory,
{
    let _span = debug_span!(
        target: "engine::tree::payload_processor::prewarm",
        "par_bal_to_hashed_post_state",
        bal_accounts = bal.len(),
        bal_total_slots = total_slots(bal),
    )
    .entered();

    let results: Result<Vec<_>, ProviderError> = bal
        .par_iter()
        .map_init(
            || provider_factory.build_reader(),
            |provider_result, account_changes| {
                let provider = match provider_result.as_deref() {
                    Ok(p) => p,
                    Err(_) => {
                        return Err(ProviderError::other(std::io::Error::other(
                            "failed to build provider for BAL conversion",
                        )))
                    }
                };
                process_account_changes(provider, account_changes)
            },
        )
        .collect();

    let partial_states = results?;

    let _span = debug_span!(
        target: "engine::tree::payload_processor::prewarm",
        "par_bal_to_hashed_post_state::merge",
    )
    .entered();

    let mut hashed_state = HashedPostState::with_capacity(bal.len());
    for partial in partial_states {
        if let Some((hashed_address, account, storage)) = partial {
            hashed_state.accounts.insert(hashed_address, Some(account));
            if let Some(storage_map) = storage {
    ...
    tracing::debug!(
        ...
        "par_bal_to_hashed_post_state complete"
    );

    Ok(hashed_state)
}

/// Trait for types that can produce an [`AccountReader`].
///
/// Used to abstract over provider builders so that each rayon thread can create its own reader.
pub(crate) trait AccountReaderFactory {
    /// Build a boxed [`AccountReader`].
    fn build_reader(&self) -> Result<Box<dyn AccountReader>, ProviderError>;
}

/// Processes a single BAL account entry: reads from the provider (to fill the cache) and
/// returns the hashed account + optional storage updates.
fn process_account_changes(
    provider: &dyn AccountReader,
    account_changes: &alloy_eip7928::AccountChanges,
) -> Result<Option<(alloy_primitives::B256, Account, Option<HashedStorage>)>, ProviderError> {
    ...
    let storage = if !account_changes.storage_changes.is_empty() {
    ...
    Ok(Some((hashed_address, account, storage)))
}
